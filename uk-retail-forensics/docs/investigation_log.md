...
**Chapter Five (complete): MRP/Inventory Signal Sprint.** Extends the project's method — SQL-confirmed findings, purpose-built 3D drill-downs — to a second business function: warehousing and purchasing. Builds `stock_behavior_fields` (4,734 SKUs, six dimensions mirroring Chapter Two's customer-side structure) directly from `full_transactions`, and confirms three headline inventory signals: 572 Overdue Restock candidates, 93 Seasonal Dormant SKUs, and 108 genuine Dead Stock candidates. See "Chapter Five: MRP/Inventory Signal Sprint," below.
...
| 151 – 180 | Chapter Five | `stock_behavior_fields` build; Overdue Restock threshold retightening; Dead Stock/Seasonal Dormant safeguard; cross-population skew comparison |
...
**⚠️ Provenance note on this section:** Unlike every chapter above, this section was reconstructed by Claude on July 30, 2026 from prior session summaries at Ree's request — not transcribed live, query by query, from the actual `/sql/151` through `/sql/180` files as the rest of this log was. The figures below (572/93/108, thresholds, row counts) are corroborated independently across two separate prior sessions and match `chapter_four_calculated_fields.md`'s already-verified Field 8-15 results exactly, so confidence is high — but per this project's own standing rule ("nothing gets called a finding on the strength of a chart alone"), this reconstruction is not a substitute for a direct read of the canonical `/sql/` files. Treat this section as a placeholder narrative pending a pass where the actual query files are pasted in and checked line-by-line, the same way queries 1-136 were.
...
**151–158** — Built `full_transactions` and the six individual stock-side fields (recency, frequency, monetary, interval, demand breadth/product diversity mirror, return rate) against it, each verified independently before assembly — same discipline as Chapter Two's customer-side field build.

**159** — `159_build_stock_behavior_fields.sql`. Assembled `stock_behavior_fields`, one row per stock_code, anchored on recency (full population, no exclusions) with the other five fields LEFT JOINed on so structurally-absent SKUs (13 cancellation-only, 103 unattributed-only) carry NULLs rather than being dropped — mirrors Query 94's customer-side assembly exactly. **Result: 4,734 rows**, matching the full stock recency population.

**162–164** — First-pass Overdue Restock definition (3x the SKU's own average order interval) and dead-stock candidate pull. **Result:** the 3x threshold flagged 1,532 SKUs — 32.4% of the entire catalog, too loose to function as a targeted signal. The dead-stock pull (frequency_completed ≤ 3, recency_days ≥ 377) returned **201 candidates**.

**165** — Seasonality safeguard. Tested whether any of the 201 dead-stock candidates were actually seasonal-only rather than genuinely dormant. **Result:** 93 of the 201 had their entire order history clustered in November/December — confirmed via multi-order clustering far beyond chance, not a coincidence of small sample size. **Confirmed finding:** these 93 are excluded from clearance recommendations and flagged for seasonal restocking instead. Genuine clearance candidates: **108** (201 − 93).

**171–174** — Retightened the Overdue Restock threshold from 3x to **8x the SKU's own average order interval, plus a £1,000 historical net-value floor** — restricting the population to SKUs both meaningfully dormant and worth purchasing's attention. **Result: 572 candidates**, a workable, targeted population comparable in scale to the dead-stock categories rather than a third of the catalog. Query 174 rebuilt the combined three-category exhibit dataset: **773 rows total — 572 Overdue Restock / 108 Dead Stock Candidate / 93 Seasonal Dormant.**

**175** — Dead Stock vs. Seasonal Dormant deep-dive dataset, introducing `avg_order_month` as a derived axis (since `frequency_completed` only takes values 1-3 for this subset and produces flat, uninformative stripes as a Z-axis). **Confirmed finding:** clean spatial separation — Seasonal Dormant clusters 11.0-12.0, Dead Stock spreads 1.0-10.25, zero overlap.

**176** — Standalone Overdue Restock exhibit data, using `overdue_multiple` (the literal ratio defining the category) as an axis instead of raw recency.

**177–179** — Tested whether the "wave/spray" visual shape from the Chapter Three customer-side cube and this sprint's Overdue Restock exhibit were the same underlying phenomenon or a coincidental shared artifact of `frequency_completed` being independently right-skewed on both sides. First attempt (177) had a customer-population-vs-filtered-stock-subset scope mismatch; corrected in 178, comparing both FULL populations: 2.08x right-skew (customer) vs. 2.36x (stock) — similar in magnitude. Query 179's follow-up histogram (10,573 rows: 5,852 customer + 4,721 stock) showed the two distributions are shaped differently despite the similar skew ratio — customer is spike-and-tail, stock is a broad hump. **Confirmed finding: the shapes are NOT the same phenomenon** — a clean example of the project's own lesson that a single summary statistic can confirm skew exists without confirming two distributions are shaped alike.

**Exhibits built this chapter (all self-contained Plotly HTML):**
- `inventory_signal_3d.html` — 773 SKUs, three categories, recency/monetary/log-frequency axes.
- `dead_stock_seasonal_deepdive_3d.html` — the two dead-stock-derived categories, recency/monetary/avg_order_month axes — the spatial proof for Query 165.
- `overdue_restock_standalone_3d.html` — 572-SKU standalone view, `overdue_multiple` as an axis.
- `frequency_distribution_comparison.html` — the Query 177-179 customer-vs-stock skew comparison.

**Chapter Five status: core findings confirmed, Tableau-side build tracked separately.** All three headline SQL findings (572/93/108) are confirmed and exhibited. The Tableau calculated fields and worksheets built on top of these findings (Fields 8-15: Overdue Multiple, Inventory Signal Category, Total/Nov-Dec Orders per SKU, Is Seasonal Dormant, Exhibit URL routing) are documented in `docs/chapter_four_calculated_fields.md` rather than duplicated here, per that document's role as source of truth for dashboard build status.
