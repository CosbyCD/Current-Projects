...
**Project window:** July 6 – July 31, 2026 (26 days, five chapters — originally scoped at 20 days/four chapters; the extra time went to re-rendering the project's volume of data-intensive 3D exhibits across multiple sessions)
...
**Phase 1 is closed as a prototype.** It proved the forensic methodology and the 3D drill-down concept hold up end-to-end — from raw data through a stakeholder-navigable dashboard — against a fixed, static dataset.

## What's next — Phase 2

The prototype's job was to de-risk the concept before building live infrastructure. It did. Phase 2 takes the same forensic pipeline — cleaning, derived behavioral fields, SQL-confirmed findings, drill-down exhibits — and rebuilds it to run against real-time data:

- **Backend:** Flask, replacing the static PostgreSQL export with a live data layer
- **Frontend:** HTML5, replacing the Tableau storyboard with a custom-built interface
- **Same standing rules carry forward** — nothing shipped without SQL confirmation, nothing silently corrected, every revision traceable to the query that made it

This phase is in active development.
